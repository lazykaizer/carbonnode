...
export default function HowItWorks() {
  ...
  const stepColors = [
    { bg: 'from-blue-500/10 to-cyan-500/10', border: 'border-blue-200/50', icon: 'from-blue-500 to-cyan-500', number: 'bg-blue-500' },
    { bg: 'from-purple-500/10 to-pink-500/10', border: 'border-purple-200/50', icon: 'from-purple-500 to-pink-500', number: 'bg-purple-500' },
    { bg: 'from-emerald-500/10 to-teal-500/10', border: 'border-emerald-200/50', icon: 'from-emerald-500 to-teal-500', number: 'bg-emerald-500' },
  ];
          ...
          {HOW_IT_WORKS_STEPS.map((step, index) => (
            ...
            >
              ...
              {index < HOW_IT_WORKS_STEPS.length - 1 && (
                <div className="hidden md:block absolute top-16 left-[60%] w-[80%] z-0" aria-hidden="true">
                  <div className="h-[2px] w-full bg-gradient-to-r from-gray-200 via-gray-300 to-gray-200 relative">
                    <div
                      ...
                      style={{ transitionDelay: prefersReducedMotion ? '0ms' : `${500 + index * 300}ms` }}
                    />
                  </div>
                </div>
              )}
              ...
              <div
                className={[
                  ...
                  prefersReducedMotion ? '' : 'hover:-translate-y-2 transition-transform duration-300'
                ].join(' ')}
              >
                {/* Step icon */}
                <div className="relative inline-flex items-center justify-center w-20 h-20 rounded-2xl bg-white shadow-lg mb-6">
                  ...
                  <span className={`absolute -top-2 -right-2 w-8 h-8 rounded-full ${stepColors[index].number} text-white text-sm font-bold flex items-center justify-center shadow-lg`}>
                    {index + 1}
                  </span>
                </div>
