...
export default function Sidebar() {
  ...
  if (isMobile) {
    return (
      <>
        <nav className="fixed bottom-0 left-0 right-0 z-40 bg-white border-t border-gray-200 shadow-lg" aria-label="Mobile navigation">
          <div className="flex items-center justify-around px-2 py-2">
            ...
            <button onClick={() => setIsAchievementsOpen(true)} className="flex flex-col items-center gap-1 px-4 py-2 cursor-pointer touch-target" aria-label="View achievements and badges">
              <span className="text-xl" aria-hidden="true">{currentLevel.emoji}</span>
  ...
  return (
    <>
      <aside className="fixed left-0 top-0 bottom-0 w-64 bg-white border-r border-gray-100 flex flex-col z-30 shadow-sm" aria-label="Sidebar navigation">
        <div className="px-6 py-5 border-b border-gray-100">
          <button onClick={() => navigate('/')} className="flex items-center gap-3 cursor-pointer" aria-label="Go to home page">
        ...
        <div className="px-4 pb-6 space-y-4">
          <button onClick={() => setIsAchievementsOpen(true)} className="w-full text-left bg-brand-bg rounded-xl p-4 cursor-pointer hover:bg-brand-bg/85 border border-transparent hover:border-brand-primary/10 transition-all duration-200" aria-label="View achievements and badges">
            ...
            <div className="h-2 bg-gray-200 rounded-full overflow-hidden" role="progressbar" aria-valuenow={Math.round(xpProgress.percentage)} aria-valuemin={0} aria-valuemax={100} aria-label={`XP progress: ${Math.round(xpProgress.percentage)}%`}>
              <div
                className={[
          ...
          <div className="flex items-center justify-between px-4 py-3 bg-orange-50 rounded-xl">
            ...
            <span className="text-lg font-bold text-orange-600 carbon-value">{streak} {streak === 1 ? 'day' : 'days'}</span>
          </div>
          ...
          <div className="flex items-center justify-between px-4 py-3 bg-brand-bg rounded-xl">
            ...
            <span className={`text-lg font-bold carbon-value ${budgetPercent > 85 ? 'text-status-danger' : budgetPercent > 60 ? 'text-status-warning' : 'text-brand-primary'}`}>{Math.round(budgetPercent)}%</span>
          </div>
        </div>
      </aside>
      <AchievementsModal isOpen={isAchievementsOpen} onClose={() => setIsAchievementsOpen(false)} />
