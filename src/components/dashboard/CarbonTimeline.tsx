  ...
  return (
    <div className="bg-card border border-border rounded-2xl p-6 shadow-sm text-left">
      ...
      <p className="text-xs text-text-secondary mb-6">
        Click on any node to view daily milestones and log itemization relative to the national average.
      </p>
      ...
      >
        <div className="min-w-[960px] h-full flex justify-between items-end relative pt-6 px-4">
          ...
          {dailyData.map((node) => {
            ...
            return (
              <div key={node.dayStr} className="flex flex-col items-center gap-2 relative z-10 flex-1">
                {/* Milestone Badge */}
                {node.milestones.length > 0 && (
                  <div className="absolute -top-7 flex gap-0.5 justify-center">
                    {node.milestones.map((ms, idx) => (
                      <span key={idx} title={ms.label} className="text-xs cursor-help bg-white p-0.5 rounded-full shadow border border-gray-100">
                        {ms.icon}
                      </span>
                    ))}
                  </div>
                ...
                <button
                  ...
                  className={`w-3 h-3 rounded-full transition-all duration-300 relative focus:outline-none cursor-pointer ${
                    ...
                    !hasData ? 'bg-gray-300 hover:bg-gray-400'
                    : isOverLimit ? 'bg-status-danger hover:scale-125'
                ...
                {hasData && (
                  <div
                    className={`w-1 rounded-t-full absolute bottom-0 pointer-events-none transition-colors duration-300 ${
                      ...
                      : isOverLimit ? 'bg-status-danger/20'
          ...
          <div className="bg-gray-50 border border-gray-150 rounded-xl p-4 space-y-3">
              <div className="flex justify-between items-center">
                <h3 className="text-sm font-bold text-text-primary">
                  Logs for {selectedNode.date.toLocaleDateString('en-IN', { weekday: 'long', day: 'numeric', month: 'long' })}
                </h3>
                <div className="text-right">
                  <span className="text-xs text-text-muted block">Daily Footprint</span>
                  <span className={`text-sm font-black ${selectedNode.totalCo2 > INDIA_URBAN_DAILY_KG ? 'text-status-danger' : 'text-status-success'}`}>
              ...
              {selectedNode.milestones.length > 0 && (
                <div className="flex flex-wrap gap-1.5 pt-1">
                  {selectedNode.milestones.map((ms, idx) => (
                    <span key={idx} className="inline-flex items-center gap-1 text-[10px] bg-white border border-gray-150 px-2 py-0.5 rounded-full text-text-secondary shadow-sm">
                      <span>{ms.icon}</span>
                      <span>{ms.label}</span>
                    </span>
                  ))}
              ...
              {selectedNode.entries.length > 0 ? (
                <div className="space-y-1.5 pt-2 max-h-48 overflow-y-auto pr-1">
                  {selectedNode.entries.map((entry: CarbonEntry) => (
                    <div key={entry.id} className="flex items-center justify-between text-xs py-1.5 border-b border-gray-100 last:border-0">
                      <div className="flex items-center gap-2 truncate pr-4">
                        <span className="text-xs" aria-hidden="true">{CATEGORY_ICONS[entry.category]}</span>
                        <span className="truncate text-text-primary font-medium">{entry.activityName}</span>
                      </div>
                      <span className="font-bold text-text-primary flex-shrink-0">{formatCo2Kg(entry.co2Kg)}</span>
                    </div>
                  ))}
                </div>
              ) : (
                <p className="text-xs text-text-muted py-2">No carbon footprint logged on this day.</p>
              )}
            </div>
          )}
        </div>
