...
describe('geminiService', () => {
  ...
  it('handles rate limiting logic locally', () => {
  ...
  it('successfullies make an API request with retries', async () => {
  ...
  it('throws FatalError on 429 response', async () => {
    ...
    await expect(analyzeDailyActivity('test')).rejects.toThrow(
      'Rate limit exceeded. Try again in 10s.',
    );
  });

  it('retries on 500 error and succeed', async () => {
    vi.useRealTimers();
    const fetchMock = global.fetch as import('vitest').Mock;
    // 1st fails with 500
    fetchMock.mockResolvedValueOnce({
  ...
  it('fails immediately on 400 error', async () => {
    const fetchMock = global.fetch as import('vitest').Mock;
    fetchMock.mockResolvedValueOnce({
      ok: false,
      status: 400,
  ...
  it('calls analyzeReceipt with correct payload', async () => {
    const fetchMock = global.fetch as import('vitest').Mock;
    fetchMock.mockResolvedValueOnce({
      ok: true,
      json: async () => ({}),
  ...
  it('calls analyzeUrl with correct payload', async () => {
    const fetchMock = global.fetch as import('vitest').Mock;
    fetchMock.mockResolvedValueOnce({
      ok: true,
      json: async () => ({}),
  ...
  it('calls generateCarbonStory with correct payload', async () => {
    const fetchMock = global.fetch as import('vitest').Mock;
    fetchMock.mockResolvedValueOnce({
      ok: true,
      json: async () => ({}),
  ...
  it('fails client-side rate limit check', async () => {
  ...
  it('caches identical requests and skip redundant network calls', async () => {
    const mockResponse = { activities: [], totalCo2Kg: 5, overallSuggestion: 'good' };
    const fetchMock = global.fetch as import('vitest').Mock;
  ...
  it('evicts from cache on request failure', async () => {
    const fetchMock = global.fetch as import('vitest').Mock;
