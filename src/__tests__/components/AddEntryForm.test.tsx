...
describe('AddEntryForm Component Tests', () => {
  it('has no axe accessibility violations', async () => {
    const { container } = render(<AddEntryForm onSuccess={() => {}} onCancel={() => {}} />);
  ...
  it('calculates transport emission correctly and submits transport entry', () => {
    ...
    // Fill description
    fireEvent.change(screen.getByLabelText(/activity description/i), {
      target: { value: 'Petrol Commute' },
    });

    // Choose Petrol Car Ride
    fireEvent.change(screen.getByLabelText(/type of transport/i), {
  ...
  it('calculates food emission correctly and awards transport streak if metro used', () => {
    ...
    fireEvent.change(screen.getByLabelText(/activity description/i), {
      target: { value: 'Metro Commute' },
    });
    fireEvent.change(screen.getByLabelText(/type of transport/i), {
      target: { value: 'metro_train' },
    });
  ...
  it('calculates food beef meal emission correctly', () => {
    ...
    fireEvent.change(screen.getByLabelText(/activity description/i), {
      target: { value: 'Beef dinner' },
    });
    fireEvent.change(screen.getByLabelText(/type of food & dining/i), {
      target: { value: 'meal_beef' },
    });
  ...
  it('calculates shopping polyester emission correctly', () => {
    ...
    fireEvent.change(screen.getByLabelText(/activity description/i), {
      target: { value: 'New t-shirt' },
    });
    fireEvent.change(screen.getByLabelText(/type of shopping/i), {
      target: { value: 'shop_polyester' },
    });
